    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if ui.button("Starting circling").clicked() {
                send_req("circle".to_owned(), self.tx.clone(), ctx.clone(), self.looping);
            }
        });
    }
}
...
fn test_turn_on(serial_conn: &mut Box<dyn SerialPort>, tx: Sender<PositionPhases>, ctx: egui::Context) {
...
fn test_turn_off(serial_conn: &mut Box<dyn SerialPort>, tx: Sender<PositionPhases>, ctx: egui::Context) {
